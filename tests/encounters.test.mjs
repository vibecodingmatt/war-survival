...
import { openChoice, resolveChoice, activatePower } from '../js/core/encounters.js';
import { flightPose } from '../js/core/flight.js';

function fresh(){const s=new Simulation();s.start(2);s.choiceTimer=Infinity;return s;}
...
test('surviving launched soldiers stop shooting, pause in flight, and recover without being recruited again',()=>{
  ...
  s.hurtCooldown=0;s.hurt(10,false,{x:0,z:5,mini:true});assert.ok(s.knockups.length);s.start(2);assert.equal(s.knockups.length,0);
});
...
test('new powers deal earned damage, pause, and clear completely on restart',()=>{
  for(const kind of ['starfall','tesla','prism']){
    ...
    s.start(2);assert.equal(s.buffs[kind],0);assert.equal(s.powersTaken,0);assert.equal(s.choice,null);assert.equal(s.fallen.length,0);
  }
});
test('rift powers expire on the battle clock without persisting into later encounters',()=>{
  ...
  for(const kind of ['starfall','tesla','prism'])activatePower(s,kind);
  step(s,10.2);assert.equal(s.state,'active');for(const kind of ['starfall','tesla','prism'])assert.equal(s.buffs[kind],0);
});
