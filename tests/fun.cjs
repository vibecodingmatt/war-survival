...
(async()=>{
 ...
 const seen=new Set();let choiceShot=false,state;
 for(let i=0;i<700;i++){
  ...
  for(const kind of ['starfall','tesla','prism'])if(state.buffs[kind]>0&&!seen.has(kind)){
   await page.evaluate(seconds=>window.__warTest.step(seconds),kind==='starfall'?.65:.35);await page.waitForTimeout(150);await page.screenshot({path:path.join(output,kind+'.png')});
   const fx=await page.evaluate(()=>window.__warTest.spectacle());if(kind==='tesla')assert.ok(fx.drones);if(kind==='prism')assert.ok(fx.prism);if(kind==='starfall')assert.ok(fx.meteors>0);seen.add(kind);
  }
 ...
 assert.equal(seen.size,3,'all three powers can be earned through normal shooting');
 if(state.state==='active'){await page.locator('#pause-button').click();await page.locator('#pause-panel [data-menu]').click();}else await page.locator('#result-panel [data-menu]').click();
 await page.locator('[data-level="9"]').click();await page.locator('#start-button').click();let launch=false;
 for(let i=0;i<700;i++){
  state=await page.evaluate(()=>window.__warTest.snapshot());if(state.state!=='active')break;
  if(state.launchedSoldiers>0&&!launch){launch=true;await page.evaluate(()=>window.__warTest.step(.45));await page.waitForTimeout(150);await page.screenshot({path:path.join(output,'boss-soldiers-airborne.png')});assert.ok((await page.evaluate(()=>window.__warTest.spectacle())).segments>0);}
 ...
 await phone.goto(base+'?test=1');await phone.waitForFunction(()=>window.__warTest?.ready);await phone.evaluate(()=>window.__warTest.useManualClock());await phone.locator('#start-button').tap();await phone.evaluate(()=>window.__warTest.step(9));await phone.waitForTimeout(300);
 ...
 assert.ok(await phone.locator('#rift-banner').isVisible());assert.match(await phone.locator('#rift-options').innerText(),/SOLDIERS.*STARFALL/);assert.ok(await phone.locator('#rift-options').evaluate(e=>parseFloat(getComputedStyle(e).fontSize)>=15));
 ...
 assert.deepEqual(errors,[]);console.log('PASS: exclusive rift boards, three earned powers, meteor/drone/prism visuals, real boss launches, and mobile choice layout.');await browser.close();
