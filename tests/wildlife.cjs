...
(async()=>{
  ...
  for(const mobile of [false,true]){
    ...
    await page.goto(base+'?test=1',{waitUntil:'domcontentloaded',timeout:60000});await page.waitForFunction(()=>window.__warTest?.ready,null,{timeout:60000});
    ...
    for(let level=0;level<15;level++){
      ...
      await page.evaluate(()=>{const w=window.__warTest.world();window.__warTest.visualTime(w.detailTime-w.wildlifeTime+20);document.querySelector('#pause-panel').style.visibility='hidden';});
      await page.waitForTimeout(250);
      let state=await page.evaluate(()=>window.__warTest.world());
      assert.ok(state.signatureCount>0,'signature creature in world '+(level+1));assert.ok(state.creatures<=14);assert.ok(state.detailBatches<=12);
      ...
      report.push({mobile,level:level+1,species:state.wildlifeSpecies,signature:state.signatureKind,behavior:state.signaturePositions[0].mode,count:state.creatures});
      if(level===0){
      ...
      await page.evaluate(()=>{const w=window.__warTest.world();window.__warTest.visualTime(w.detailTime-w.wildlifeTime+55);});
      const later=await page.evaluate(()=>window.__warTest.world());
      assert.equal(later.butterflyBloom,0,'gatherings have quiet intervals');
      if(state.signaturePositions[0].mode==='resting')assert.deepEqual(later.signaturePositions,state.signaturePositions,'resting creatures remain on their perch');
  ...
  assert.equal(new Set(report.filter(r=>!r.mobile).map(r=>r.signature)).size,15);
  assert.deepEqual(errors,[]);fs.writeFileSync(path.join(output,'report.json'),JSON.stringify(report,null,2));
  console.log('PASS: 15 exclusive fantasy creatures, grounded resting poses, volcanic dragons, butterfly gathering, desktop/phone framing and pause.');
  await browser.close();
